/*
 Markdown-it Library.
 This library belongs to markdown-it team but is kept and edited to meet requirements of Sanatan AI
 Edited version for Sanatan AI.
 The code may be breaking rules mentioned in license,
 Owner will not be responsible for this.
*/
...
function markdownit() {
  ...
  */
  ...
  @param {any} exclude - list of characters to ignore (in addition to a-zA-Z0-9)
  @param {boolean} keepEscaped   - don't encode '%' in a correct escape sequence (default: true)
...
/**
   ...
   * ```
   * @param {'Commonmark'|'zero'} presetName: optional, `commonmark` / `zero`
   ...
   * ##### options:
   *
   * - html - `false`. Set `true` to enable HTML tags in source. Be careful!
   *   That's not safe! You may need external sanitizer to protect output from XSS.
   *   It's better to extend features via plugins, instead of enabling HTML.
   * - xhtmlOut - `false`. Set `true` to add '/' when closing single tags
   *   (`<br />`). This is needed only for full CommonMark compatibility. In real
   *   world you will need HTML output.
   * - breaks - `false`. Set `true` to convert `\n` in paragraphs into `<br>`.
   * - langPrefix - `language-`. CSS language class prefix for fenced blocks.
   *   Can be useful for external highlighters.
   * - linkify - `false`. Set `true` to autoconvert URL-like text to links.
   * - typographer  - `false`. Set `true` to enable [some language-neutral
   *   replacement](https://github.com/markdown-it/markdown-it/blob/master/lib/rules_core/replacements.mjs) +
   *   quotes beautification (smartquotes).
   * - quotes - `“”‘’`, String or Array. Double + single quotes replacement
   *   pairs, when typographer enabled and smartquotes on. For example, you can
   *   use `'«»„“'` for Russian, `'„“‚‘'` for German, and
   *   `['«\xA0', '\xA0»', '‹\xA0', '\xA0›']` for French (including nbsp).
   * - highlight - `null`. Highlighter function for fenced code blocks.
