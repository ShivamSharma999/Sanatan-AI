...
export function getPopup(): [HTMLDivElement, HTMLDivElement, HTMLTextAreaElement, HTMLButtonElement, HTMLInputElement, HTMLDivElement, HTMLButtonElement, HTMLButtonElement]
 {
    if (document.querySelector('div.chatbot-popup')) return
    const userName = localStorage.getItem('Name') || t('guestName');
    const h3 = Create("h3", {}, `${t('greetingHello')} ${userName}`),
        h2 = Create("h2.logoTxt.animated-gradient-text.fast-transition", {}, t("searchExplore")),
        wrapper = Create("div.imgWrapper"),
        home = Create("div.home", {}, [wrapper, h3, h2]);
 Create("img", {
    ...
    const deleteMenuItem = Create("button.chat-options-item.danger", {}, [
        ...
        if (window.deleteSession && window.state.activeSessionId) window.deleteSession(new Event("click"), window.state.activeSessionId);
    })]);
    ...
    return [(popup as HTMLDivElement), (chatBody as HTMLDivElement), (messageInput as HTMLTextAreaElement), (sendMessage as HTMLButtonElement), fileInput, (filePreview as HTMLDivElement), (deepThinkBtn as HTMLButtonElement), (googleBtn as HTMLButtonElement)];
}
...
function getSettings() {
    ...
    const exportBtn = Create("button.btn-gradient.conversation-tool-btn", { type: "button" }, "Export", ["click", () => {
        const format = (document.getElementById("conversation-export-format") as HTMLSelectElement)?.value;
        if (window.exportActiveConversation) window.exportActiveConversation(format as any);
    }]);
    ...
    const importInput = Create("input", { type: "file", id: "conversation-import-input", accept: "application/json,.json" }, "", ["change", function () {
        if (window.importConversationJson) window.importConversationJson(this.files?.[0]);
    }]);
