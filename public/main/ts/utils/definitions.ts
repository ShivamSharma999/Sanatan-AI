 ...
 */
export function $<K extends keyof HTMLElementTagNameMap>(query: K | string, parentElement: ParentNode = document): HTMLElementTagNameMap[K] | null {
...
export function closE(query: string) {
...
export function initDom() {
  ...
  themeTag = ($('meta[name="theme-color"]') as HTMLMetaElement);
  fullscreen = $('.fullscreen')

  welcomeContainer = $(".welcome");
  steps = document.querySelectorAll(".welcome-step");
  agreeBtn = ($("#agree-btn") as HTMLButtonElement);
  finishSetupBtn = ($("#finish-setup-btn") as HTMLButtonElement);
  welcomeNameInput = ($("#welcome-name-input") as HTMLInputElement);

  menuopen = ($(".menuopen") as HTMLButtonElement);
  googleLoginBtn = ($("#google-login-btn") as HTMLButtonElement);
  userProfileImg = ($("#user-profile-img") as HTMLImageElement);
  userNameDisplay = $("#user-name-display");
  verificationStatus = $("#verification-status");

  stopRespond = ($(".stopRespond")) as HTMLButtonElement;
