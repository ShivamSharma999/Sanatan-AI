    ...
    chats.forEach((chat) => {
        ...
        if (chat.role === "user") {
            ...
            chat.parts.forEach((part) => {
                ...
                if (part.text) {
                    ...
                    if (mimeType.startsWith("image/")) {
                        let div = Create("div", { align: "right" }), src = `data:${mimeType};base64,${data}`;
                        div.appendChild(Create("img.attachment", { src: src }));
                        fileAttachments.push(div);
                    }
                    else {
                        let span = Create("span.sanatan-symbol", {}, "description"), para = Create("p.file-attachment", {}, [span]);
            ...
            if (!hasFunctionResponse) {
                ...
                userCount++;
                const msgTxt = Create("div.message-text", {}, escapeHtml(textContent)), editMsg = Create("code.icon.sanatan-symbol", {}, "edit", ["click", () => { edit(editMsg, msgIdx); }]), delMsg = Create("code.icon.sanatan-symbol", {}, "delete", ["click", () => deleteChat(msgIdx)]), reGenBtn = Create("code.icon.sanatan-symbol", {}, "refresh", ["click", () => regenerateResponse(msgIdx + 1)]), edits = Create("p.flex", {}, [editMsg, reGenBtn, delMsg]), userMessage = Create("div.user-message.col", {}, [msgTxt, ...fileAttachments, edits]);
            ...
            if (!hasFunctionResponse) {
                ...
                botCount++;
                const msgTxt = Create("div.message-text", {}, botMessageContentHtml), logo = Create("img", {
                    src: "./Resources/images/logo.png",
                    alt: "Sanatan Logo"
                }), cDivArr = isMobile ? [msgTxt] : [logo, msgTxt], continerDiv = Create("div.row", {}, cDivArr), speakBtn = Create("code.icon.sanatan-symbol", {
                    style: {
                        marginLeft: '40px'
                    }
                }, "volume_up", ["click", () => speak(speakBtn)]), copyBtn = Create("code.icon.sanatan-symbol", {}, "content_copy", ["click", () => copyMessage(msgIdx, copyBtn)]), deleteBtn = Create("code.icon.sanatan-symbol", {}, "delete", ["click", () => { deleteChat(msgIdx); }]), reGenBtn = Create("code.icon.sanatan-symbol", {}, "refresh", ["click", () => regenerateResponse(msgIdx)]), editContainer = Create("div.row", {}, [speakBtn, copyBtn, deleteBtn, reGenBtn]), botMessage = Create("div.message.bot-message", {}, [continerDiv, editContainer]);
