...
export let stopRespond, themeTag, synth, isPC, welcomeContainer, steps, agreeBtn, finishSetupBtn, welcomeNameInput, menuopen, googleLoginBtn, isMobile = window.innerWidth < 712, menuBar = $('.menuBar'), userProfileImg, userNameDisplay, verificationStatus, fullscreen;
...
export function showAlert(alert, isPerm = false) {
    ...
    let p = Create("p", {}, alert), btn, content, chArr = [p];
    if (!isPerm) {
...
export function initDom() {
    ...
    if (enterSelect)
        enterSelect.value = localStorage.getItem("WhenEnter") || "no";
    if (nameInput)
        nameInput.value = localStorage.getItem("Name") || "user";
    if (googleBtn) {
        evListener("click", googleBtn, () => btnHov(googleBtn, state.isGoogle));
    }
    if (deepThinkBtn) {
        evListener("click", deepThinkBtn, () => btnHov(deepThinkBtn, state.isDeepChat));
    }
