...
export async function getUser() {
    if (!state.userName && !localStorage.getItem("Email"))
        return initApp();
    else if (!state.userName && localStorage.getItem("Email"))
...
export async function saveChatHistory() {
    if (!state.activeSessionId || !state.userName)
        return;
    try {
...
export function storeAIMemory(data) {
    ...
    if (!state.aiMemory.includes(str))
        state.aiMemory.push(str);
