...
import { state, createNewSessionData, loadSessionData, deleteSessionData, saveChatHistory as saveStateHistory, getUser} from "../core/state.js";
...
export async function loadSession(id) {
  if (id === state.activeSessionId && state.chatHistory.length > 0 && chatBody.innerHTML !== "") return; // Already loaded
...
async function deleteSession(event, id) {
  try {
    ...
    if (!(await Sanatan.ask("Are you sure you want to delete this chat?"))) return;
...
export function renderChatList() {
  ...
  state.chatSessions.forEach(session => {
    ...
    if (!session.timestamp) session.timestamp = parseInt(session.id);
...
export function updateProfileUI() {
  ...
  if (userNameDisplay) userNameDisplay.textContent = name || "Guest";
  if (userProfileImg && picture) userProfileImg.src = picture || "Resources/images/logo.png";
  if (nameInput && name) nameInput.value = name;
  if (welcomeNameInput && name) welcomeNameInput.value = name;
}
...
function a(b) {
  ...
  b.forEach((file) => {
    ...
    if (
      ...
      if (!allowedExtensions.includes(file.type)) file.type = "text/plain";
      ...
      reader.onload = (e) => {
        const c = {
          ...
          filename: file.name,
          isImage: isImage,
          wholeData: e.target.result,
        };
        // Check duplication by filename since object ref prevents .includes() working effectively on new objects
...
function changeTheme(isLightMode = document.body.classList.toggle("light_mode"), isAutoMode = document.querySelector("#theme-select").value === "auto") {
  if (isAutoMode) {
    detectTheme();
  } else {
...
window.speak = function(speakButton) {
  const sText =
    speakButton.parentElement.parentElement.querySelector(
      ".message-text"
    ).textContent;
  voiceControl(sText);
}
...
export async function deleteChat(i) {
  if (
    state.chatHistory[i].parts[state.chatHistory[i].parts.length - 1].functionCall &&
...
evListener("load", window, () => {
  setTimeout(() => {
    const adDiv = document.querySelector(
      'div[style*="position: fixed !important; bottom: 0px !important;"]'
    );
...
export function checkKey (event) {
  if (
    (event.key === "Enter" && !event.ctrlKey && enterSelect.value === "yes") ||
