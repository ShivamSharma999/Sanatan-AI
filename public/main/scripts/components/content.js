...
export async function generateBotResponse(wasError = false) {
    ...
    updateFilePreview();
    let answer = "", i = state.chatHistory.length - (wasError ? 1 : 0);
    const config = {
    ...
    const { botCount } = convertToHtml(state.chatHistory), e = document.querySelectorAll(".bot-message"), elem = e[botCount - 1].querySelector(".message-text");
    ...
    if (!state.activeSessionId)
        return showNotification(t("pleaseSelectChat"), "error");
    try {
