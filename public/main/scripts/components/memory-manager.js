...
export function showMemoryManager() {
    ...
    const renderList = () => {
        ...
        state.aiMemory.forEach((mem, _) => {
            ...
            const item = Create("div.memory-item.center-flex", { style: "background: rgba(255,255,255,0.05); padding: 10px; border-radius: 6px; justify-content: space-between;" }, `<span style="flex: 1; font-size: 0.9rem;">${mem}</span>`, [delBtn]);
